'''
    Simulation the running time and end-to-end delay of different algorithms (MINLP, CPEG, CNE, CCN, MPCN) over different network scales.
...
def save_summary_results_to_csv(results, filename='summary_results.csv'):
    with open(filename, 'w', newline='') as csvfile:
...
def main():
    ...
    for nodes, edges in network_scales:
        ...
        for i in range(num_runs):
            print(f"  Executing test round {i+1}...")
            ...
            for name, path in algorithms.items():
                ...
                try:
                    ...
                    print(f"      {name} algorithm error: {str(e)}")
